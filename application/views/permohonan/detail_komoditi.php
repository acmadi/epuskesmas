          ...
          <div class="col-md-6">
            ...
            <div class="box box-success">
              ...
              <div class="form-group">
                <label>Lokasi :</label>
                <textarea class="form-control" placeholder="Alamat" name="address" rows="2" /><?php echo $lokasi; ?></textarea>
              </div>
          ...
          <div class="col-md-6">
            <div class="box box-success">
              ...
              <div class="form-group">
                ...
                <select class="form-control" name="retribusi"/></select>
              </div>
              <div class="form-group">
                <label>Spesifikasi :</label>
                <select class="form-control" name="spesifikasi"/></select>
              </div>
              <div class="form-group">
                <label>Satuan :</label>
                <select class="form-control" name="satuan"/></select>
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" placeholder="Jumlah Diajukan" name="jumlah" value="<?php echo $jml; ?>"/>
              </div>
              <div class="form-group">
                <label>Asal Benih:</label>
                <input type="text" class="form-control" placeholder="Asal Benih (daerah)" name="asal_benih" value="<?php echo $asal; ?>" />
              </div>
              <div class="form-group">
                <label>Kelas Benih:</label>
                <input type="text" class="form-control" placeholder="Kelas Benih" name="kelas_benih" value="<?php echo $kelas; ?>" />
              </div>
              <div class="form-group">
                <label>Umur Benih:</label>
                <input type="text" class="form-control" placeholder="Umur Benih (bulan)" name="umur_benih" value="<?php echo $umur; ?>" />
              </div>
              <br>
...
$(function(){
    ...
    //   document.location.href="<?php echo base_url().'permohonan/komoditi_update/{id}';?>";
    // });

    $('#btn-save').click(function(){
      $.ajax({ 
        type: "POST",
        url: "<?php echo base_url().'permohonan/komoditi_update/{kode_permohonan}/{kode_varietas}/{kode_komoditi}';?>",
    ...
    $.get('<?php echo base_url();?>permohonan/retribusi/{kode_komoditi}/{kode_retribusi}', function(response) {
      var data = eval(response);
      $("select[name='retribusi']").html(data.retribusi);
    }, "json");

    $.get('<?php echo base_url();?>permohonan/spesifikasi/{kode_komoditi}/{kode_retribusi}/{kode_spek}', function(response) {
      var data = eval(response);
      $("select[name='spesifikasi']").html(data.spesifikasi);
    }, "json");

    $.get('<?php echo base_url();?>permohonan/satuan/{kode_komoditi}/{kode_retribusi}/{kode_spek}/{kode_satuan}', function(response) {
      var data = eval(response);
      $("select[name='satuan']").html(data.satuan);
    }, "json");
