<?php
class Industri_master extends CI_Controller {
	...
	function index()
	{
		$this->authentication->verify('industri_master','edit');
		$data['title'] = "MASTER DATA INDUSTRI";
		$data['content_print'] = $this->get_daftar_cetak();
		$data['content'] = $this->parser->parse("industri_master/show",$data,true);
	...
	function add()
	{
		$this->authentication->verify('industri_master','add');
		$data['title']	= "Master Data Industri &raquo; Tambah";
		$data["act"]	= "add";
		$data["pabrik"] = array();
		$data["kantor"] = array();
		$data['content'] = $this->parser->parse("industri_master/form_industri",$data,true);
		$this->template->show($data,"home");
	}
	...
	function doadd()
	{
		...
		if($this->form_validation->run()== FALSE){
			...
			echo "1__".$this->industri_master_model->insert_entry();
		}
	}

	function dodel($id_industri=0){
		$this->authentication->verify('industri_master','del');

		if($this->industri_master_model->delete_entry($id_industri)){
			echo "1";
		}else{
			echo "Delete Error";
		}
	...
	function view($id_industri="",$tab=0)
	{
		$this->authentication->verify('industri_master','edit');
		$data = $this->industri_master_model->get_data_row($id_industri); 
		$data['title']	= "Master Data Industri &raquo; Ubah : ".$data['nama_industri'];
		$data["act"]	= "edit";
		$data["id"]		= $id_industri;
		$data["tab"]	= $tab;
		...
		$data['content_print'] = $this->get_detail_cetak($data,$data['pabrik'],$data['kantor']);
		$data['content'] = $this->parser->parse("industri_master/form_industri",$data,true);
		$this->template->show($data,"home");
	}
	...
	function tab($id_industri,$tab = "tab2")
	{
		...
		$data["pabrik"] = $this->industri_master_model->get_list_pabrik($id_industri);
		$data["kantor"] = $this->industri_master_model->get_list_kantor($id_industri); 
		$data['content'] = $this->parser->parse("industri_master/form_industri_".$tab,$data,true);
		die($data['content']);
	}
	...
	function add_pabrik($id_industri="")
	{
		...
		$data["act"]	= "add_pabrik";
		$data["id"]		= $id_industri;
		die($this->parser->parse("industri_master/form_industri_pabrik",$data));
	}
	...
	function edit_pabrik_jenis($id_industri="",$id_plant=0)
	{
		...
		$data["id"]			= $id_industri;
		$data["id_plant"]	= $id_plant;
		die($this->parser->parse("industri_master/form_industri_pabrik_jenis",$data));
	}
	...
	function edit_pabrik_fasilitas($id_industri="",$id_plant=0)
	{
		...
		$data["id"]			= $id_industri;
		$data["id_plant"]	= $id_plant;
		die($this->parser->parse("industri_master/form_industri_pabrik_fasilitas",$data));
	}
	...
	function add_jenis($id_industri="",$id_plant=0)
	{
		...
		$data["id_plant"]	= $id_plant;
		$data["timestamp"]	= time();
		die($this->parser->parse("industri_master/form_industri_jenis",$data));
	}
	...
	function add_fasilitas($id_industri="",$id_plant=0)
	{
		...
		$data["id_plant"]	= $id_plant;
		$data["timestamp"]	= time();
		die($this->parser->parse("industri_master/form_industri_fasilitas",$data));
	}
	...
	function edit_jenissediaan($id_industri="",$id_plant=0,$id_jenis=0)
	{
		...
		$data["id_jenis"]	= $id_jenis;
		$data["timestamp"]	= time();
		die($this->parser->parse("industri_master/form_industri_bentuksediaan",$data));
	}
	...
	function add_kantor($id_industri="")
	{
		...
		$data["act"]	= "add_kantor";
		$data["id"]		= $id_industri;
		die($this->parser->parse("industri_master/form_industri_kantor",$data));
	}
	...
	function add_direksi($id_industri="")
	{
		...
		$data["id"]		= $id_industri;
		die($this->parser->parse("industri_master/form_industri_direksi",$data));
	}
	...
	function add_bentuksediaan($id_industri="",$id_plant=0,$id_jenis=0)
	{
		...
		$data["id_jenis"]	= $id_jenis;
		$data["timestamp"]	= time();
		die($this->parser->parse("industri_master/form_industri_bentuksediaan_form",$data));
	}
	...
	function add_izin($id_industri="")
	{
		...
		$data["act"]	= "add_izin";
		$data["id"]		= $id_industri;
		die($this->parser->parse("industri_master/form_industri_izin",$data));
	}
	...
	function json(){
		...
		{			
			...
			foreach ($jdata['records']->result() as $row){
				...
				$jvEdit="<a href='javascript:void(0);'><img border=0 src='".base_url()."public/images/edt.gif' onclick=document.location='industri_master/view/".$row->id_industri."'> </a>";
